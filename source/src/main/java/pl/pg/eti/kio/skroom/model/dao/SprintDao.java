...
import static pl.pg.eti.kio.skroom.model.Tables.SPRINTS;
...
public class SprintDao {
    ...
    public List<Sprint> fetchSprintsForProject(
        ...
        Result<SprintsRecord> sprintsRecords = query
            .selectFrom(SPRINTS)
    ...
    public List<Sprint> fetchAvailableSprintsForProject(
        Connection connection,
        Project project
    ) {
        DSLContext query = DSL.using(
        ...
        Result<SprintsRecord> sprintsRecords = query
            ...
            .where(SPRINTS.PROJECT_ID.eq(project.getId()))
            .and(
                SPRINTS.START_DAY.le(LocalDate.now())
            )
            .fetch();
    ...
    public boolean createFirstSprint(
        Connection connection,
        Project project,
        String firstSprintName
    ) {
        DSLContext query = DSL.using(
            connection,
            DatabaseSettings.getCurrentSqlDialect()
        ...
        Date end = Date.from(
            date
                .plusWeeks(project.getDefaultSprintLength())
        ...
        int insertedRows = query
            .insertInto(SPRINTS)
            .values(null, firstSprintName, start, end, project.getId())
            .execute();
        /*
     ...
     */
    public boolean removeSprintsForProject(
        Connection connection,
        Project project
    ) {
        DSLContext query = DSL.using(
    ...
    public boolean updateSprint(Connection connection, Sprint sprint) {
        ...
        int updatedRows = query
            ...
            .set(SPRINTS.START_DAY, LocalDate.from(sprint.getStartDate().toInstant()))
            .set(SPRINTS.END_DAY, LocalDate.from(sprint.getEndDate().toInstant()))
    ...
    public int getSprintCountForProject(
        Connection connection,
        Project project
    ) {
        DSLContext query = DSL.using(
        ...
        Record1<Integer> countRecord = query
            .selectCount()
            .from(SPRINTS)
            .where(SPRINTS.PROJECT_ID.eq(project.getId()))
            .fetchOne();
    ...
    public Sprint getLastSprint(Connection connection, Project project) {
        ...
        SprintsRecord sprintRecord = query
            .selectFrom(SPRINTS)
            .where(SPRINTS.PROJECT_ID.eq(project.getId()))
            .groupBy(SPRINTS.END_DAY)
            .orderBy(SPRINTS.END_DAY.desc())
            .fetchAny();
