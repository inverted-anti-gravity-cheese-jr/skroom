...
import pl.pg.eti.kio.skroom.model.tables.records.UserStoryStatusRecord;
...
public class UserStory {
     ...
     */
    public static UserStory fromDba(
        UserStoriesRecord record,
        DSLContext query
    ) {
        if (record == null) {
            return null;
        ...
        userStory.setStoryPoints(
            StoryPoints.fromValue(record.getStoryPoints())
        );

        UserStoryStatusRecord userStoryStatusRecord = query
            .selectFrom(Tables.USER_STORY_STATUS)
            .where(Tables.USER_STORY_STATUS.ID.eq(record.getStatusId()))
            .fetchOne();
