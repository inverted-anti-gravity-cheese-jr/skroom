...
import static pl.pg.eti.kio.skroom.model.Tables.USER_STORIES;
import static pl.pg.eti.kio.skroom.model.Tables.USER_STORY_STATUS;
...
import pl.pg.eti.kio.skroom.model.tables.records.UserStoriesRecord;
...
public class UserStoryDao {
    ...
    public int countUserStoriesWithStatus(
        Connection connection,
        int userStoryStatusId
    ) {
        ...
        Record1<Integer> record = query
            .selectCount()
            .from(USER_STORIES)
            .where(USER_STORIES.STATUS_ID.eq(userStoryStatusId))
            .fetchAny();

        return record.value1();
    }

    public boolean removeUserStoryStatusWithId(
        Connection connection,
        int userStoryStatusId
    ) {
        DSLContext query = DSL.using(
        ...
        int removedRows = query
            .deleteFrom(USER_STORY_STATUS)
            .where(USER_STORY_STATUS.ID.eq(userStoryStatusId))
            .execute();

        return removedRows > 0;
    }

    public boolean updateUserStoryStatuses(
        Connection connection,
        List<UserStoryStatus> statuses
    ) {
        DSLContext query = DSL.using(
            connection,
            DatabaseSettings.getCurrentSqlDialect()
        ...
        for (UserStoryStatus status : statuses) {
            if (status.getId() < 0) {
                ...
                int updated = query
                    .update(USER_STORY_STATUS)
                    .set(USER_STORY_STATUS.STATUS_NAME, status.getName())
                    .set(USER_STORY_STATUS.COLOR, status.getColor())
                    .set(
                        USER_STORY_STATUS.IS_ARCHIVE,
                        status.isArchive() ? 1 : 0
                    )
                    .where(USER_STORY_STATUS.ID.eq(status.getId()))
                    .execute();
     ...
     */
    public boolean removeUserStoryWithId(
        Connection connection,
        int userStoryId
    ) {
        DSLContext query = DSL.using(
        ...
        int removedRows = query
            ...
            .where(USER_STORIES.ID.eq(userStoryId))
            .execute();
     ...
     */
    public boolean insertNewUserStory(
        Connection connection,
        UserStory userStory,
        Project project
    ) {
        ...
        int insertedRows = query
            .insertInto(USER_STORIES)
            .values(
                null,
                userStory.getName(),
                userStory.getAsA(),
                userStory.getiWantTo(),
                userStory.getSoThat(),
                userStory.getDescription(),
                userStory.getPriority(),
                userStory.getStoryPoints().getValue(),
                userStory.getStatus().getId(),
                project.getId()
            )
            .execute();
    ...
    public boolean updateUserStory(Connection connection, UserStory userStory) {
        ...
        int updatedRows = query
            ...
            .where(USER_STORIES.ID.eq(userStory.getId()))
            .execute();
     ...
     */
    public ArrayList<UserStory> fetchUserStoriesForProject(
        Connection connection,
        Project project
    ) {
        DSLContext query = DSL.using(
        ...
        Result<UserStoriesRecord> userStoriesRecords = query
            .selectFrom(USER_STORIES)
            .where(USER_STORIES.PROJECT_ID.eq(project.getId()))
            .fetch();
    ...
    public UserStory fetchUserStoryById(Connection connection, int id) {
        ...
        UserStoriesRecord userStoryRecord = query
            .selectFrom(USER_STORIES)
            .where(USER_STORIES.ID.eq(id))
            .fetchOne();
     ...
     */
    public boolean removeUserStoriesForProject(
        Connection connection,
        Project project
    ) {
        DSLContext query = DSL.using(
