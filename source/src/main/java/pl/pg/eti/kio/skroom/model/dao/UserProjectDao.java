...
import pl.pg.eti.kio.skroom.model.tables.records.UsersProjectsRecord;
...
public class UserProjectDao {
    ...
    public List<UserProjectContainer> listAllUsersForProject(
    ...
    private List<UserProjectContainer> getUserProjectContainers(
        ...
        try {
            for (Record record : records) {
                Integer userProjectId = record
                    .into(USERS_PROJECTS)
                    .getProjectId();
                User user = User.fromDba(record.into(USERS));
                UserRolesInProject role = UserRolesInProject.fromDba(
                    record.into(USER_ROLES_IN_PROJECT)
                );
                results
                    .computeIfAbsent(user.getId(), k ->
                        new UserProjectContainer(userProjectId, user)
                    )
                    .getRoles()
                    .add(role);
            }
    ...
    public Optional<UserProjectContainer> fetchContainerById(
        Connection connection,
        int projectId,
        int userId
    ) {
    ...
    public List<User> availableUsersForProject(
        Connection connection,
        int projectId
    ) {
        DSLContext query = DSL.using(
    ...
    public boolean checkIfUserIsAlreadyInProject(
        Connection connection,
        User user,
        int projectId
    ) {
        ...
        Result<UsersProjectsRecord> records = query
            .selectFrom(USERS_PROJECTS)
