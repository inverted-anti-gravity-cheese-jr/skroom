...
import static pl.pg.eti.kio.skroom.model.Tables.TASKS;
import static pl.pg.eti.kio.skroom.model.Tables.TASK_STATUSES;
import static pl.pg.eti.kio.skroom.model.Tables.USERS;
...
public class TaskDao {
    ...
    public void updateTaskStatus(
        ...
        query
            .update(TASKS)
            .set(TASKS.TASK_STATUS_ID, taskStatusId)
    ...
    public List<Task> fetchTasks(
        ...
        Result<UsersRecord> usersRecords = query
            .selectFrom(USERS)
            .where(USERS.ID.in(tasksRecords.getValues(TASKS.ASSIGNEE)))
            .fetch();
        ...
        try {
            ...
            for (TasksRecord taskRecord : tasksRecords) {
                ...
                if (taskRecord.getAssignee() != null) {
                    user = records.usersRecordsList
                        .stream()
                        .filter(x -> x.getId() == taskRecord.getAssignee())
                        .findAny()
                        .get();
                }
                Task task = Task.fromDba(
                    taskRecord,
                    user,
                    project,
                    taskStatuses,
    ...
    public boolean moveTasksToCurrentSprint(
        Connection connection,
        Sprint sprint
    ) {
        DSLContext query = DSL.using(
        ...
        SelectConditionStep<Record1<Integer>> selectStep = query
            .select(TASKS.ID)
        ...
        int movedTasks = query
            .update(TASKS)
            .set(TASKS.SPRINT_ID, sprint.getId())
            .where(TASKS.ID.in(selectStep))
            .execute();

        return movedTasks >= 0;
    }

    public boolean changeTaskAssignee(
        Connection connection,
        int id,
        User user
    ) {
        ...
        int changedRows = query
            .update(TASKS)
            .set(TASKS.ASSIGNEE, user.getId())
    ...
    public Task fetchTaskById(
        ...
        if (tasksRecord.getAssignee() != null) {
            user = users
                .stream()
                .filter(u -> u.getId() == tasksRecord.getAssignee())
                .findAny()
                .get();
        }
    ...
    public Task fetchTaskByIdForCurrentUser(
        Connection connection,
        int id,
        User user,
        Project project,
    ...
    public boolean createTask(Connection connection, Task task) {
        ...
        Integer assigneeId = (task.getAssignee() != null
    ...
    public int countTasksForStatus(Connection dbConnection, int taskStatusId) {
        ...
        Record1<Integer> count = query
            .selectCount()
            .from(TASKS)
            .where(TASKS.TASK_STATUS_ID.eq(taskStatusId))
            .fetchAny();
