...
public class InitDao {
    ...
    public void createDatabase() {
        String dialect = DatabaseSettings.getCurrentSqlDialect()
            .getName()
            .toLowerCase();
        Connection dbConnection = DatabaseSettings.getDatabaseConnection();
        try {
            Statement stmt = dbConnection.createStatement();
            int numberOfUsers = stmt
            ...
            LOGGER.info(
                "Could not open database, generating new one {}",
                e.getMessage()
            );
            try {
                ...
                LOGGER.info(
                    "Could not init database, reason {}",
                    ex.getMessage()
                );
            }
        }
    ...
    private void generateInitialData(Connection connection, String dialect)
        throws Exception {
        String ddlSql = readSqlFile("default-" + dialect + ".sql");
        executeMultipleStatements(connection, ddlSql);
    }

    private void generateTables(Connection connection, String dialect)
        throws Exception {
