...
import static pl.pg.eti.kio.skroom.model.Tables.*;
...
import pl.pg.eti.kio.skroom.model.tables.records.UsersSettingsRecord;
...
public class UserDao {
    ...
    public boolean saveUserSettings(
        ...
        DSLContext query = DSL.using(
            connection,
            DatabaseSettings.getCurrentSqlDialect()
        );
        ...
        int updatedRows = query
            ...
            .set(
                USERS_SETTINGS.USER_STORIES_PER_PAGE,
                userSettings.getUserStoriesPerPage()
            )
     ...
     */
    public List<UserContainer> listAllUsers(
        Connection connection,
        ...
        SelectConditionStep<Record> whereStep = query
            .select(USERS.fields())
            .select(Tables.USERS_SECURITY.ACCEPTED)
            .from(USERS)
            .join(Tables.USERS_SECURITY)
            .onKey()
        ...
        try {
            for (Record record : records) {
                ...
                Record1<Integer> accepted = record.into(
                    Tables.USERS_SECURITY.ACCEPTED
                );
                users.add(
                    new UserContainer(
                        User.fromDba(userRecord),
                        accepted.value1() == 1
                    )
                );
            }
     ...
     */
    public List<User> listAllUsersForProject(
        Connection connection,
        Project project
    ) {
        DSLContext query = DSL.using(
        ...
        SelectConditionStep<Record1<Integer>> usersInProject = query
            .selectDistinct(USERS_PROJECTS.USER_ID)
            .from(USERS_PROJECTS)
            .where(USERS_PROJECTS.PROJECT_ID.eq(project.getId()));

        Result<UsersRecord> records = query
            .selectFrom(USERS)
            .where(USERS.ID.in(usersInProject))
            .fetch();
     ...
     */
    public boolean checkIfHasProjectEditPermissions(
        Connection connection,
        User user,
        Project project
    ) {
        ...
        Record1<Integer> rolesWithPrivilages = query
            .selectCount()
            .from(Tables.USERS_PROJECTS, Tables.USER_ROLES_IN_PROJECT)
            .where(
                Tables.USERS_PROJECTS.USER_ID.eq(user.getId())
                    .and(
                        Tables.USERS_PROJECTS.PROJECT_ID.eq(
                            project.getId()
                        ).and(
                            Tables.USER_ROLES_IN_PROJECT.ID.eq(
                                Tables.USERS_PROJECTS.USER_ROLE_ID
                            )
                        )
                    )
                    .and(
                        Tables.USER_ROLES_IN_PROJECT.PRIVILIGES.eq(
                            USER_PRIVILAGE_TO_EDIT_PROJECT
                        )
                    )
            )
            .fetchOne();
    ...
    public User fetchByName(Connection connection, String name) {
        ...
        Result<UsersRecord> usersRecordResult = query
            .selectFrom(USERS)
    ...
    public UserContainer fetchUserContainerByName(
        Connection connection,
        String name
    ) {
        ...
        Record1<Integer> accepted = query
            .select(USERS_SECURITY.ACCEPTED)
            .from(USERS_SECURITY)
            .where(USERS_SECURITY.USER_ID.eq(user.getId()))
            .fetchOne();
    ...
    public boolean assignToProject(
        Connection connection,
        User user,
        Project project,
        UserRolesInProject role
    ) {
        DSLContext query = DSL.using(
            connection,
            DatabaseSettings.getCurrentSqlDialect()
        ...
        int newRows = query
            .insertInto(USERS_PROJECTS)
            .values(null, user.getId(), project.getId(), role.getId())
            .execute();
     ...
     */
    public UserSecurity fetchUserSecurityData(
        Connection connection,
        User user
    ) {
        DSLContext query = DSL.using(
        ...
        UsersSecurityRecord usersSecurityRecord = query
            .selectFrom(Tables.USERS_SECURITY)
            .where(Tables.USERS_SECURITY.USER_ID.eq(user.getId()))
            .fetchOne();
     ...
     * @return
     */
    public UserSettings fetchUserSettingsData(
        Connection connection,
        User user
    ) {
        DSLContext query = DSL.using(
        ...
        UsersSettingsRecord usersSettingsRecord = query
            .selectFrom(USERS_SETTINGS)
            .where(USERS_SETTINGS.USER_ID.eq(user.getId()))
            .fetchOne();
    ...
    public void registerUser(
    ...
    ) throws UserAlreadyExistsException, UserAccountCreationErrorException {
        DSLContext query = DSL.using(
            connection,
            DatabaseSettings.getCurrentSqlDialect()
        );

        int usersWithTheSameNameOrEmailCount = query
            .selectCount()
            .from(USERS)
            .where(
                USERS.NAME.eq(user.getName()).or(
                    USERS.EMAIL.eq(user.getEmail())
                )
            )
        ...
        try {
            query.transaction(configuration -> {
                Integer userId = (Integer) DSL.using(configuration)
                    .fetchOne(
                ...
                int usersCreatedRows = DSL.using(configuration)
                    .insertInto(USERS)
                    .values(
                        null,
                        user.getName(),
                ...
                int usersSecurityCreatedRows = query
                    .insertInto(Tables.USERS_SECURITY)
                    .values(
                        null,
                        userId.intValue() + 1,
                        userSecurity.getPassword(),
                        userSecurity.getSalt(),
                        userSecurity.getSecureQuestion(),
                        userSecurity.getSecureAnswer(),
                        0
                    )
                    .execute();

                int usersSettingsCreateRows = query
                    .insertInto(USERS_SETTINGS)
                    .values(null, userId.intValue() + 1, -1, 10, 25, 5)
                    .execute();

                if (
                    usersCreatedRows != 1 ||
                    usersSecurityCreatedRows != 1 ||
                    usersSettingsCreateRows != 1
                ) {
                    // rollback
                    throw new Exception();
                }
