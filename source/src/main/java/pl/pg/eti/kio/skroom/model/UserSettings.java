...
public class UserSettings {
     ...
     */
    public static UserSettings fromDba(
        UsersSettingsRecord record,
        DSLContext query
    ) {
        if (record == null) {
            return null;
        ...
        if (
            record.getRecentProjectId() != null &&
            record.getRecentProjectId().intValue() != -1
        ) {
            ProjectsRecord projectRecord = query
                .selectFrom(Tables.PROJECTS)
                .where(
                    Tables.PROJECTS.ID.eq(
                        record.getRecentProjectId().intValue()
                    )
                )
                .fetchOne();
