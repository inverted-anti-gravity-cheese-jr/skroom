...
public class AccountManagementController {
    ...
    private static final Logger LOGGER = Logger.getLogger(
        AccountManagementController.class.getName()
    );

    // Messages
    private static final String YOUR_ACCOUNT_MUST_BE_ACCEPTED =
        "Your account hasn't been accepted yet. Please contact your administrator.";
    private static final String WRONG_USERNAME_OR_PASSWORD_MESSAGE =
        "You entered wrong username or password. Please try again.";
    private static final String SUCCESSFULLY_CREATED_ACCOUNT =
        "Thank you, your account has been created successfully.";

    // Redirects
    private static final String REDIRECT_AFTER_SUCCESSFULL_LOGIN =
        "redirect:/dashboard";
    private static final String PASSWORDS_NOT_MATCH = "Passwords not match";
    private static final String WRONG_EMAIL_FORMAT =
        "Wrong email format. Use email in format <email>@<domain>.<suffix>";
    ...
    public ModelAndView editUser(
        ...
        ModelAndView model = injector.getIndexForSiteName(
            USER_FORM_JSP_LOCATION,
            "Edit user - Skroom",
            userSettings.getRecentProject(),
            user,
            webRequest
        );
        ...
        UserDao.UserContainer editUser = userDao.fetchUserContainerByName(
            dbConnection,
            editUserName
        );
        ...
        model.addObject(
            "availableUserRoles",
            UserRole.getAvailableDisplayNames()
        );
    ...
    public ModelAndView editUser(
        ...
        @ModelAttribute("userSettings") UserSettings userSettings,
        @PathVariable String editUserName,
        @RequestParam String editUserRole
    ) {
        if (user == null || user.getId() < 0) return new ModelAndView(
            "redirect:/"
        );
    ...
    public ModelAndView tryLoggingIn(
        @ModelAttribute("loggedUser") User user,
        ...
        UserSecurity userSecurity = userDao.fetchUserSecurityData(
            dbConnection,
            user
        );
        ...
        if (userSecurity.getPassword().equals(password)) {
            UserSettings settings = userDao.fetchUserSettingsData(
                dbConnection,
                user
            );

            ModelAndView model = new ModelAndView(
                REDIRECT_AFTER_SUCCESSFULL_LOGIN
            );
    ...
    public ModelAndView createAccount(
        ...
        if (!confirmPassword.equals(password)) {
            LOGGER.log(Level.FINEST, "Passwords not match for user {}.", new Object[] { name } );
            return getSignupModelWithErrorMessage(PASSWORDS_NOT_MATCH);
        }

        if (!email.matches("(.*@.*[.].*)")) {
        	LOGGER.log(Level.FINEST, "Wrong email format {}.", new Object[] { email });
