...
import static pl.pg.eti.kio.skroom.model.Tables.PROJECTS;
...
public class ProjectDao {
    ...
    public Project fetchProjectById(Connection connection, int id) {
        ...
        ProjectsRecord projectRecord = query
            .selectFrom(PROJECTS)
    ...
    public boolean addProject(
        ...
        int inserted = query
            ...
            .values(
                ...
                project.getDefaultSprintLength()
            )
            .execute();

        if (inserted == 1) {
            Result<Record> projIdQuery = query.fetch(
    ...
    public void updateProject(Connection connection, Project project) {
        ...
        query
            ...
            .set(
                PROJECTS.DEFAULT_SPRINT_LENGTH,
                project.getDefaultSprintLength()
            )
            .where(PROJECTS.ID.equal(project.getId()))
            .execute();
    }
     ...
     */
    public boolean checkUserViewPermissionsForProject(
        Connection connection,
        Project project,
        User user
    ) {
        ...
        resultPrivilege = query
            .select(Tables.USER_ROLES_IN_PROJECT.PRIVILIGES)
     ...
     * @return Returns true if user has any privileges that allow him to edit project, and false otherwise
     */
    public boolean checkUserEditPermissionsForProject(
        Connection connection,
        Project project,
        User user
    ) {
        ...
        return userDao.checkIfHasProjectEditPermissions(
            connection,
            user,
            project
        );
     ...
     * @return Project class from the model
     */
    public Project getProjectForUser(
        Connection connection,
        int projectId,
        User user
    ) {
        ...
        if (
            !checkUserViewPermissionsForProject(connection, tempProject, user)
        ) {
            return null;
        }
    ...
    public List<Project> getProjectsForUser(Connection connection, User user) {
        ...
        if (UserRole.ADMIN.equals(user.getRole())) {
            ...
            result = query
                ...
                .where(
                    Tables.USERS_PROJECTS.USER_ID.eq(user.getId()).and(
                        PROJECTS.ID.eq(Tables.USERS_PROJECTS.PROJECT_ID)
                    )
                )
                .fetch();
        }
