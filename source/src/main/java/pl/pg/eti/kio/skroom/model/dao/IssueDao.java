...
import pl.pg.eti.kio.skroom.model.tables.records.TaskStatusesRecord;
...
public class IssueDao {
    ...
    public List<Issue> listAllIssues(Connection connection, Project project) {
        ...
        Result<Record> records = query
            .select()
            .from(ISSUES)
            .leftJoin(TASK_STATUSES)
            .onKey()
            .leftJoin(USERS)
            .onKey()
            .leftJoin(TASKS)
        ...
        try {
            for (Record record : records) {
                ...
                if (record.get(ISSUES.STATUS_ID) != null) {
                    status = TaskStatus.fromDba(
                        record.into(TASK_STATUSES),
                        project
                    );
                }
                User assignee = null;
                ...
                results.add(
                    Issue.fromDba(issue, status, assignee, task, project)
                );
            }
    ...
    private Map<Integer, Task> taskMapForProject(
        Connection connection,
        Project project
    ) {
        List<UserStory> userStories =
            new UserStoryDao().fetchUserStoriesForProject(connection, project);
        List<TaskStatus> taskStatuses = new TaskStatusDao().fetchByProject(
            connection,
            project
        );
        List<Sprint> sprints = new SprintDao().fetchAvailableSprintsForProject(
            connection,
            project
        );
        List<Task> taskList = new TaskDao().fetchTasks(
            connection,
            project,
            taskStatuses,
            userStories,
    ...
    public Issue fetchById(
        Connection connection,
        int issueId,
        Project project
    ) {
        //TODO change
        return listAllIssues(connection, project)
            .stream()
            .filter(x -> x.getId() == issueId)
            .findFirst()
            .orElse(null);
    }
    ...
    public TaskStatus fetchStatusByName(
        Connection connection,
        ...
        List<TaskStatusesRecord> records = query
    ...
    public List<TaskStatus> availableIssueStatuses(
        Connection connection,
        Project project
    ) {
        DSLContext query = DSL.using(
        ...
        Result<TaskStatusesRecord> records = query
            .selectFrom(TASK_STATUSES)
            .where(TASK_STATUSES.PROJECT_ID.eq(project.getId()))
            .fetch();
        ...
        return records
            .stream()
            .map(x -> TaskStatus.fromDba(x, project))
            .collect(Collectors.toList());
    }

    public List<User> availableIssueUsers(
        Connection connection,
        Project project
    ) {
        DSLContext query = DSL.using(
    ...
    public User fetchUserNyName(
        Connection connection,
        String userName,
        Project project
    ) {
        return availableIssueUsers(connection, project)
            .stream()
    ...
    public List<Task> availableIssueTasks(
        Connection connection,
        Project project
    ) {
    ...
    public Task fetchTaskByName(
        Connection connection,
        String taskName,
        Project project
    ) {
        return availableIssueTasks(connection, project)
            .stream()
