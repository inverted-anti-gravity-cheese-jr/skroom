...
public class TaskStatusDao {
    ...
    public TaskStatus fetchByName(
        ...
        return TaskStatus.fromDba(
            query
                .selectFrom(TASK_STATUSES)
                .where(TASK_STATUSES.NAME.eq(name))
                .and(TASK_STATUSES.PROJECT_ID.eq(project.getId()))
                .fetchAny(),
            project
        );
    }

    public List<TaskStatus> fetchByProject(
        Connection connection,
        Project project
    ) {
        DSLContext query = DSL.using(
        ...
        Result<TaskStatusesRecord> taskStatusesRecords = query
            .selectFrom(TASK_STATUSES)
    ...
    public boolean generateDefaultStatusesForProject(
        Connection connection,
        Project project
    ) {
    ...
    public boolean removeById(Connection connection, int tsId) {
        ...
        int deletedRows = query
            .deleteFrom(TASK_STATUSES)
            .where(TASK_STATUSES.ID.eq(tsId))
            .execute();

        return deletedRows > 0;
    }

    public boolean createTaskStatus(
        Connection connection,
    ...
    public boolean updateTaskStatuses(
        Connection connection,
            ...
            query.transaction(t -> {
                for (TaskStatus taskStatus : taskStatuses) {
                    ...
                    if (taskStatus.getId() < 0) {
                        results = createTaskStatus(connection, taskStatus)
                            ? 1
                            : 0;
                    } else {
                        results = nquery
                    ...
                    if (results < 1) {
                        throw new Exception(
                            "Rollback on value " + taskStatus.toString()
                        );
                    }
                }
            });
