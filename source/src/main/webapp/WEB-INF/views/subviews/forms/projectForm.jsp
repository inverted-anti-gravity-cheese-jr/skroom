...
<%@taglib
prefix="s" uri="/WEB-INF/skroom-tags.tld"%>
...
<form method="post" class="skroom-form">
	...
	<div class="management-bar" style="margin-bottom: 5px;">
		...
		<c:if test="${ not empty project }">
			<a href="removeProject/${project.id}" class="btn red pull-right">Delete project</a>
		</c:if>
	</div>

	<label>Project name</label>
	<input name="projectName" placeholder="Project name" value="${project.name}"
		class="form-control" />
	<label>Project description</label>
	<textarea name="projectDescription" placeholder="Project description"
		class="form-control">
		<s:html2plain>${project.description}</s:html2plain>
	</textarea>
	<label>Sprint length in weeks</label>
	<div class="input-group">
		<input name="sprint-length" placeholder="Sprint length" class="form-control"
	...
	<c:if test="${ createProject eq 'true' }">
		...
		<input name="first-sprint-name" placeholder="Name of a first sprint"
			value="Sprint 1" class="form-control" />
	</c:if>
